...
DEFAULT_DATA_PARAMS = {
    ...
    "dataset_type": '1',
    "data_path": CLEAN_2020_HUMAN_DATA_1
}
...
def gen_accs():
    acc_arr = []    
    for i in range(34):
        model, params = load_bc_model(os.path.join(BC_SAVE_DIR, str(i + 1)))
        model_acc = []
        for j in range(34):
            data_path = os.path.join(CLEAN_HUMAN_DATA_DIR, "2020_hh_trials_player" + str(j+1) + ".pickle")
            params["data_params"]["data_path"] = data_path
            inputs, seq_lens, targets = load_data(params, True)
            acc = model.evaluate(inputs, targets)[1]
            model_acc.append(acc)
            # print('model #' + str(i + 1) + ' with data #' + str(j + 1) + ' has accuracy: ' + str(acc))
        acc_arr.append(model_acc)
    np.savetxt('heatmap.txt', acc_arr)

def plot_heatmap():
    accs = np.loadtxt('heatmap.txt')
    fig, ax = plt.subplots()
    im = ax.imshow(accs)
    players = np.arange(34)

    # Show all ticks and label them with the respective list entries
    ax.set_xticks(np.arange(len(players)), labels=players)
    ax.set_xlabel("Player to Predict for", fontsize=18)
    ax.set_yticks(np.arange(len(players)), labels=players)
    ax.set_ylabel("Player Used for Model Training", fontsize=18)

    for i in range(len(players)):
        square = plt.Rectangle((i - 0.5, i - 0.5), 1, 1, fc=None, ec="black", lw=2, fill=False)
...
def plot_boxplot():
    ...
    ax1.boxplot(accs)
    # print('means: ', np.mean(accs, axis=0), 'stdevs: ', np.std(accs, axis=0))
    plt.show()
...
def plot_bars():
    ...
    players = np.arange(34)
    fig1, ax1 = plt.subplots()
    #ax1.bar(players, np.mean(accs, axis=0))
    ax1.bar(players, np.std(accs, axis=0))
    plt.show()
